...
> **Security — Vulnerability Scanner** — installed software CVE lookups via OSV + NVD
...
- **Network Map** — ARP sweep + ping fallback for LAN host discovery, structured traceroute
...
### 🛡️ Security
- **Vulnerability Scanner** — installed software matched against OSV + NVD CVE databases with SQLite caching and local service banner grabbing
...
### 🔧 Developer / Admin
- **Python & PowerShell Snippet Runners** — isolated subprocess execution with stdout/stderr capture and timeout enforcement
...
The app starts a local Flask server on `127.0.0.1:5000` and launches a native desktop window via pywebview. All tool results are stored in a central `STATE` dictionary and passed to Jinja2 templates on every render. Results are also persisted to JSON cache files so previous scans survive app restarts.
